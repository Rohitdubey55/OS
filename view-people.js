      ...
      <div class="people-wrapper">
        <style>
            ...
            .people-list-item {
                ...
                background: var(--surface-1); border-radius: 12px; margin-bottom: 2px;
...
function renderPeopleGrid(people) {
    ...
    return `<div class="grid" style="grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap:6px;">
...
function renderPeopleTimeline(people) {
    ...
    if (withDates.length > 0) {
        ...
        Object.keys(grouped).forEach(month => {
            html += `<h3 style="margin:12px 0 6px; color:var(--text-muted); font-size:14px;">${month}</h3>`;
    ...
    if (withoutDates.length > 0) {
        html += `<h3 style="margin:12px 0 6px; color:var(--text-muted); font-size:14px;">No Next Interaction Set</h3>`;
        html += `<div class="people-timeline">`;
...
function renderPersonListItem(p) {
    ...
    <div class="people-list-item" onclick="window.openEditPerson('${p.id}')">
        <div class="people-list-avatar">
            ${(p.name || '?').charAt(0).toUpperCase()}
        </div>
        <div style="flex:1; min-width:0;">
            <div style="font-weight:600; font-size:13px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${p.name} ${isFav ? '⭐' : ''}</div>
            <div style="font-size:10px; color:var(--text-muted);">${p.relationship || 'Contact'}</div>
        </div>
        <div style="text-align:right; font-size:10px; flex-shrink:0;">
            <div>L: ${lastContact}</div>
        </div>
        ${balance !== 0 ? `<div class="${balanceClass}" style="font-weight:600; font-size:12px; margin-left:4px;">${balance > 0 ? '+' : ''}₹${balance}</div>` : ''}
    </div>`;
}
    ...
    <div class="card person-card person-card-${isExpanded ? 'expanded' : 'collapsed'}" style="position:relative; border:1px solid var(--border-color); border-radius:12px; overflow:hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.02); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); background: linear-gradient(180deg, var(--surface-1) 0%, var(--surface-2) 100%); margin-bottom: 2px;">
       <div class="person-card-header" onclick="window.togglePersonCard('${p.id}')" style="cursor:pointer; display:flex; justify-content:space-between; align-items:center; padding:4px 8px; transition:background 0.2s;">
           <div style="display:flex; gap:6px; align-items:center; min-width:0;">
               <div class="avatar" style="width:28px; height:28px; border-radius:6px; background:linear-gradient(135deg, var(--primary-soft), var(--primary)); color:white; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:12px; box-shadow: 0 4px 10px rgba(79, 70, 229, 0.2); flex-shrink:0;">
                   ${(p.name || '?').charAt(0).toUpperCase()}
               </div>
           ...
           <div style="display:flex; align-items:center; gap:6px;">
                <button class="btn icon" onclick="event.stopPropagation(); window.toggleFavoritePerson('${p.id}', ${!isFav})" title="${isFav ? 'Remove from favorites' : 'Add to favorites'}" style="padding: 4px;">
                    <i data-lucide="star" style="width:16px; ${isFav ? 'fill:var(--warning); color:var(--warning)' : 'color:var(--text-muted)'}"></i>
                </button>
                <i data-lucide="chevron-down" class="person-expand-icon" style="width:20px; color:var(--text-muted); ${isExpanded ? 'transform:rotate(180deg);' : ''}"></i>
            </div>
        </div>
        
        <div class="person-card-body" style="${isExpanded ? 'max-height:500px; opacity:1;' : 'max-height:0; opacity:0;'} overflow:hidden; transition:all 0.3s ease; margin-top:4px; font-size:13px; color:var(--text-secondary)">
