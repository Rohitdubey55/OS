...
window.startTutorSession = async function(topicId) {
  ...
  try {
    ...
    console.error('[Tutor] Mic access error:', e);
  ...
  // NOTE: Do NOT specify sampleRate — iOS only supports native rate (48kHz).
  // AudioContext auto-resamples when playing buffers created at different rates (24kHz).
  let playbackCtx;
  try {
    ...
    console.error('[Tutor] AudioContext error:', e);
    showToast('Audio setup failed: ' + (e.message || 'unknown'), 'error');
    // Stop mic since we can't proceed
...
function playNextTutorChunk() {
  ...
  // Play via AudioContext (pre-created in gesture handler for iOS — no forced sampleRate)
  const audioCtx = _tutor.playbackCtx || new (window.AudioContext || window.webkitAudioContext)();
