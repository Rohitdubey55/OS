...
const ICON_MAPPINGS = {
  ...
  'check': { lucide: 'check', emoji: '✓', material: 'check', remix: 'check-line', tabler: 'check', fontawesome: 'fa-check', heroicons: 'check', feather: 'check' },
  'lock': { lucide: 'lock', emoji: '🔒', material: 'lock', remix: 'lock-line', tabler: 'lock', fontawesome: 'fa-lock', heroicons: 'lock-closed', feather: 'lock' },
  'unlock': { lucide: 'unlock', emoji: '🔓', material: 'lock_open', remix: 'lock-unlock-line', tabler: 'lock-open', fontawesome: 'fa-lock-open', heroicons: 'lock-open', feather: 'unlock' },
  ...
  'party-popper': { lucide: 'party-popper', emoji: '🎉', material: 'celebration', remix: 'cake-3-line', tabler: 'confetti', fontawesome: 'fa-party-horn', heroicons: 'gift', feather: 'gift' },
...
// --- Lazy CDN loader for icon-pack CSS ----------------------------------------
// Only fetches the network resources for the pack the user actually selected.
// Default (lucide / emoji / heroicons) loads nothing.
const ICON_PACK_CDN = {
    material: 'https://fonts.googleapis.com/icon?family=Material+Icons+Outlined',
    ...
    feather:  'https://cdn.jsdelivr.net/npm/feather-icons@4.29.1/dist/feather.min.css'
};
const _loadedIconPacks = new Set();
function ensureIconPackLoaded(pack) {
    if (!pack || _loadedIconPacks.has(pack)) return;
    const href = ICON_PACK_CDN[pack];
    if (!href) { _loadedIconPacks.add(pack); return; } // lucide/emoji/heroicons need no CSS
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = href;
    link.dataset.iconPack = pack;
    link.onload  = () => console.log('[IconPacks] Loaded CDN for', pack);
    link.onerror = () => console.warn('[IconPacks] Failed to load CDN for', pack);
    document.head.appendChild(link);
    _loadedIconPacks.add(pack);
}
window.ensureIconPackLoaded = ensureIconPackLoaded;

// Get current icon pack from settings (with fallback to lucide)
function getCurrentIconPack() {
  ...
  try {
    ...
    return settings.icon_pack || 'lucide';
  } catch (e) {
    return 'lucide';
  }
}
...
// Boot: lazy-load the current pack's CDN once we know what it is.
// state.data isn't populated yet on first script eval, so wait for DOMContentLoaded
// (which fires after main.js sets up state) plus a tick for safety.
(function bootIconPackLoader() {
    function loadCurrent() {
        try { ensureIconPackLoaded(getCurrentIconPack()); }
        catch (e) { console.warn('[IconPacks] boot load failed:', e); }
    }
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => setTimeout(loadCurrent, 50));
    } else {
        setTimeout(loadCurrent, 50);
    }
    // Re-check after data loads, in case the user has a non-default pack saved.
