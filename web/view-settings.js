...
function renderSettings() {
  ...
  main.innerHTML = `
    <div class="settings-wrapper">
      ...
      <details class="settings-details" style="display:block;">
        <summary class="widget-header" style="cursor:pointer; padding:16px 20px; margin:0; background:var(--surface-1); border-bottom:1px solid var(--border-color); border-radius:16px 16px 0 0; list-style:none;">
            <div class="widget-title">${renderIcon('lock', null, 'style="width:18px; margin-right:8px;"')} Privacy &amp; Security</div>
            ${renderIcon('down', null, 'style="width:20px; transition:transform 0.3s;"')}
        </summary>
        <div class="widget-body" style="padding:20px; border-radius:0 0 16px 16px; background:var(--surface-1);">
            <p class="section-description">Protect your diary, finance, and notes data.</p>

            <div class="setting-item">
                <label class="setting-label">Biometric Lock</label>
                <label class="toggle">
                    <input type="checkbox" id="biometricLockToggle"
                           ${(typeof window !== 'undefined' && window.biometricLock?.isEnabled?.()) ? 'checked' : ''}
                           onchange="toggleBiometricLock(this.checked)">
                    <span class="slider"></span>
                </label>
            </div>
            <p class="setting-hint">
                Requires fingerprint or face authentication to open the app.
                Re-locks after 60 seconds in the background.
                Falls back gracefully if your device doesn't support biometrics.
            </p>

            <div class="setting-item" style="margin-top:12px">
                <button class="btn secondary" onclick="window.biometricLock?.lockNow?.()">
                    ${renderIcon('lock', null, 'style="width:14px; margin-right:6px;"')} Lock Now
                </button>
                <span class="permission-status" id="biometricLockStatus" style="margin-left:10px; color:var(--text-muted); font-size:12px;"></span>
            </div>
        </div>
      </details>
...
// Flips the localStorage flag that biometric-lock.js reads on app launch.
// Surfaces a confirmation toast and a status note in the Settings UI.
window.toggleBiometricLock = function (enabled) {
    const status = document.getElementById('biometricLockStatus');
    if (!window.biometricLock) {
        if (status) status.textContent = 'Lock module not loaded';
        if (typeof showToast === 'function') showToast('Biometric lock unavailable');
        return;
    }
    if (enabled) {
        window.biometricLock.enable();
        if (status) status.textContent = 'Enabled — will prompt on next launch / resume';
        if (typeof showToast === 'function') showToast('Biometric lock enabled');
    } else {
        window.biometricLock.disable();
